...
uint32_t CANSimple::service_stack() {
    ...
    for (auto& a : axes) {
        MEASURE_TIME(a.task_times_.can_heartbeat) {
            ...
            if (a.config_.can.idq_rate_ms > 0) {
                if ((now - a.can_.last_idq) >= a.config_.can.idq_rate_ms) {
                    if (get_iq_callback(a))
                        a.can_.last_idq = now;
                }

                int nextAxisService = a.can_.last_idq + a.config_.can.idq_rate_ms - now;
                nextServiceTime = std::min(nextServiceTime, static_cast<uint32_t>(std::max(0, nextAxisService)));
            }
        }
    }
