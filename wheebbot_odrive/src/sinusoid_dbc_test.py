...
t0 = time.monotonic()
tf=t0+15 #15s simulation
while (time.monotonic() - t0)<=(tf-t0):
    setpoint = 4.0 * math.sin((time.monotonic() - t0)*0.6)
    print("goto " + str(setpoint))
    print("\n current time:"+str(time.monotonic()- t0))
...
#writing to txt for plotting
sim_array=np.zeros((10, 2))
sim_file=open("steps_dbc_test.txt", "w") #open file to write data
for row in sim_array:
    np.savetxt(sim_file, row)
sim_file.close()
