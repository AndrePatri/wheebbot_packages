...
bool use_also_board_IMU=false; // whether to use the IMU on the micro controller
...
{
  uint32_t temp;
  Serial.print("\n Calibration status (0-3):\t"); Serial.println(src_p->orient_cal_status085, 8);
  Serial.print("IMU temperature:\t"); Serial.println(src_p->temperature, 8);
  ...
  memcpy(&temp, &src_p->temperature, sizeof(temp));
  dst_p[2] |= pack_left_shift_u32(temp, 0u, 0xffu);
  dst_p[3] |= pack_right_shift_u32(temp, 8u, 0xffu);
  dst_p[4] |= pack_right_shift_u32(temp, 16u, 0xffu);
  dst_p[5] |= pack_right_shift_u32(temp, 24u, 0xffu);
...
void loop() {
  ...
  if (bno08x.getSensorEvent(&sensorValue)) {
    ...
    switch (sensorValue.sensorId) {
     ...
     // Sending (hopefully calibrated) gyroscope read from bno055
