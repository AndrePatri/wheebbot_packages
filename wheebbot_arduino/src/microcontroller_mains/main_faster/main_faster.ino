/////////////////////////////////////////////// MAIN WITHOUT UNESSENTIAL SENSOR DATA (no heartbeat message) --> with the Nano IoT allows a dt around 7 ms ///////////////////////////////////////////////////


// PERFORMANCE (Nano Iot) depending on the chosen orientation report type:
// SH2_GYRO_INTEGRATED_RV ---> interval between Heartbeats is approx. 3-7 ms (peaks of 10 ms are possible)
// SH2_ARVR_STABILIZED_RV ---> interval between Heartbeats is approx. 7 ms
// SH2_ROTATION_VECTOR ---> interval between Heartbeats is approx. 7 ms
// SH2_GEOMAGNETIC_ROTATION_VECTOR ---> interval between Heartbeats is approx. 7 ms
// SH2_GAME_ROTATION_VECTOR ---> interval between Heartbeats is approx. 7 ms
...
struct enabled_reports {
  ...
  const bool IS_SH2_GYRO_INTEGRATED_RV = true; // Top frequency is reported to be 1000Hz (but freq is somewhat variable); potentially less accurate, but provides also a calibrated gyroscope reading
  ...
  const bool IS_SH2_GAME_ROTATION_VECTOR = false; // no magnetometer (no yaw jumps); yaw reference fixed at startup
