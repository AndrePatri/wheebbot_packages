/////////////////////////////////////////////// COMPARING LIN ACCELERATION PERFORMANCE BETWEEN BNO055 AND BNO085 (BNO055 offers much better gravity readings !!!!!) /////////////////////////////////////////////// 
...
struct enabled_reports {
  ...
  const bool IS_SH2_LINEAR_ACCELERATION = true; // total acceleration - gravity
...
sensors_event_t  bno055Grav;
sensors_event_t  bno055LinAcc;
...
void setReports(struct enabled_reports enbld_rep, struct IMU_report_dts reportDts) {
  ...
  if (enbld_rep.IS_SH2_ACCELEROMETER == true) {
    bno08x.enableReport(SH2_ACCELEROMETER,reportDts.ACCELEROMETER_dt); // Setting report type/types and report rate 
    }
  if (enbld_rep.IS_SH2_GYROSCOPE_CALIBRATED == true) {
    bno08x.enableReport(SH2_GYROSCOPE_CALIBRATED,reportDts.GYROSCOPE_CALIBRATED_dt); // Setting report type/types and report rate
    }
  if (enbld_rep.IS_SH2_MAGNETIC_FIELD_CALIBRATED == true) {
    bno08x.enableReport(SH2_MAGNETIC_FIELD_CALIBRATED,reportDts.MAGNETIC_FIELD_CALIBRATED_dt);  // Setting report type/types and report rate
    }
  if (enbld_rep.IS_SH2_LINEAR_ACCELERATION == true) {
    bno08x.enableReport(SH2_LINEAR_ACCELERATION,reportDts.LINEAR_ACCELERATION_dt); // Setting report type/types and report rate
    }
  if (enbld_rep.IS_SH2_GRAVITY == true) {
    bno08x.enableReport(SH2_GRAVITY,reportDts.GRAVITY_dt); // Setting report type/types and report rate
    }
  if (enbld_rep.IS_SH2_ROTATION_VECTOR == true) {
    bno08x.enableReport(SH2_ROTATION_VECTOR,reportDts.ROTATION_VECTOR_dt); // Setting report type/types and report rate
    }
  ...
  if (enbld_rep.IS_SH2_RAW_ACCELEROMETER == true) {
    bno08x.enableReport(SH2_RAW_ACCELEROMETER,reportDts.RAW_ACCELEROMETER_dt); // Setting report type/types and report rate
    }
  if (enbld_rep.IS_SH2_RAW_GYROSCOPE == true) {
    bno08x.enableReport(SH2_RAW_GYROSCOPE,reportDts.RAW_GYROSCOPE_dt);  // Setting report type/types and report rate
    }
  if (enbld_rep.IS_SH2_RAW_MAGNETOMETER == true) {
    bno08x.enableReport(SH2_RAW_MAGNETOMETER); // Setting report type/types and report rate
    }
}
...
void loop() {
  ...
  if (bno08x.getSensorEvent(&sensorValue)) {
    ...
    // Building the Heartbeat message
    ...
    switch (sensorValue.sensorId) {
      ...
      case SH2_GYRO_INTEGRATED_RV:{
       ...
       case SH2_ARVR_STABILIZED_RV:{
        ...
        case SH2_LINEAR_ACCELERATION:{
          ...
          src_p_lin_acc_first_half.x_ddot = sensorValue.un.linearAcceleration.x;
          src_p_lin_acc_first_half.y_ddot = sensorValue.un.linearAcceleration.y;
          src_p_lin_acc_second_half.z_ddot = sensorValue.un.linearAcceleration.z;
    
          sendLinAccOverCAN(dst_p_lin_acc_first_half, dst_p_lin_acc_second_half, &src_p_lin_acc_first_half, &src_p_lin_acc_second_half, &messageIDs);
     ...
     // Sending (hopefully calibrated) gyroscope readings from bno055

     bno.getEvent(&bno055Gyro, Adafruit_BNO055::VECTOR_LINEARACCEL);// using raw gyro message to actually encode gravity from BNO055 (for simplicity)
     
     src_p_raw_gyro_first_half.gyro_x= bno055Gyro.acceleration.x; 
     src_p_raw_gyro_first_half.gyro_y= bno055Gyro.acceleration.y;
     src_p_raw_gyro_second_half.gyro_z= bno055Gyro.acceleration.z;
